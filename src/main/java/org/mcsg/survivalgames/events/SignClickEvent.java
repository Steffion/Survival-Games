...
public class SignClickEvent implements Listener{
    ...
    public void clickHandler(PlayerInteractEvent e){
        ...
        if(lines[0].equalsIgnoreCase(ChatColor.GOLD + "[" + ChatColor.RED + "MCHG" + ChatColor.GOLD + "]")) {
            e.setCancelled(true);
            try{
                if(lines[2].equalsIgnoreCase(ChatColor.AQUA + "Auto Assign")){
                    GameManager.getInstance().autoAddPlayer(e.getPlayer());
                }
                else{
                    String game = lines[2].replace(ChatColor.GREEN + "Arena ", "");
